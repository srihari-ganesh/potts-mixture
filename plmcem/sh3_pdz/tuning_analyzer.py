...
msa_file_name = 'sh3_pdz_combined.fa'
true_cluster_file_names = ['sh3_trimmed.fa', 'pdz_trimmed.fa']
...
#    df_dict = {column: [] for column in columns[:6]}
    ...
    for i, param_set in enumerate(product(*param_lists)):
        if param_set[4] == 'kmeans':
            continue
            #added since kmeans succeeds in clustering on this anyway, and EM ends after 1 iteration
