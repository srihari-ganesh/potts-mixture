...
parser.add_argument('-em', '--em-maxiter', default=30, help='Maximum number of iterations of the EM algorithm.')
...
parser.add_argument('-t', '--theta', help='PLMC alignment processing: Sequence weights: neighborhood divergence [0 < t < 1].') #are we assuming default is 0.8?
...
#For ease of tuning job submission, where parameters below are redundant with flags above; basically accepts the flag as an argument so all arguments can have the same format
parser.add_argument('-gt', '--gapignore-tune', help='Tuning: gapignore, 1 or 0')
parser.add_argument('-sgdt', '--fast-tune', help='Tuning: fast, 1 or 0')
...
def set_onehot_dict(config):
    '''
    Creates dictionary for conversion between alphabet and onehot encodings
    Relevant input: config['gapignore']
    Output: Letter-to-onehot conversion dictionary
    '''
...
def configure_config(config):
    '''
    Formats input arguments from config file. Also sets up some objects that are used throughout (onehot dictionary, PyTorch device)
    '''

    #if tuning parameters are given
    if config['gapignore_tune'] != None:
        config['gapignore'] = config['gapignore_tune']
    if config['fast_tune'] != None:
        config['fast'] = config['fast_tune']
    ...
    config['onehot_dict'] = set_onehot_dict(config)
    return config
...
def kmeans_assignments(msa, num_clusters, config):
    '''
    Given MSA object, returns a list of assignments that results from kmeans clustering.
    '''
    result = skcluster.KMeans(n_clusters = num_clusters).fit(msa.onehot_msa(config).T)
...
def random_assignments(msa, num_clusters, config):
    '''
...
def initialize(config):
    '''
    Read MSA and initialize clusters
    '''
    ...
    assignments, assignment_list = assignment_method(msa, num_clusters, config)
...
def run_plmc(args):
    ...
    '''
    #note that I don't have focus enabled in PLMC, just figured it made sense since we're worried about the full alignment
...
def reassign(msa, param_file_names, msa_name, iteration, config):
    '''
    Creates new clusters by assigning sequences to the cluster with which it has the highest pseudolikelihood
    '''
    num_clusters = config['em_numclusters']
...
def run_em(config):
    ...
    with open(info_file_name, 'w') as f:
        ...
        if em_iter_print:
            print('\nIteration 0:')
        #run plmc on each of the initial clusters (maximization)
        ...
        for i in range(1, num_iters+1):
            if em_iter_print:
                print('\nIteration ' + str(i) + ':')
            t = dtn()
            #create new clusters by assigning sequences based on pseudolikelihood (expectation)
