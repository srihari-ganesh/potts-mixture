...
msa_file_name = ###Fill in MSA file name###
true_cluster_file_names = ###Fill in with list of the file names of the alignments containing the true clusters###

def main(msa_file_name_head):
...
#    df_dict = {column: [] for column in columns[:6]}
    ...
    for i, param_set in enumerate(product(*param_lists)):
        ...
        if np.all([os.path.exists(file_name) for file_name in inferred_cluster_file_names]):
            ...
            for inferred_cluster_ind in range(2):
                for true_cluster_ind in range(2):
                    df_dict['i' + str(inferred_cluster_ind) + ':t' + str(true_cluster_ind)].append('NA')
            df_dict['score'].append('Run failed')
            #print('Run failed')
    ...
    open('tuning_analysis.txt', 'w').write(tabulate(df, headers = 'keys'))
